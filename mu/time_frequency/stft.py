...
from ..reconstruction.overlap import (overlap_add, ola)
from ..core import frame
# Authors : David C.C. Lu <davidlu89@gmail.com>
#
# License : BSD (3-clause)

def stft(x, binsize, hopsize, window='hamming', mode=None, **kwargs):
    fft_ = fft.fft if mode is 'full' else fft.rfft
    win_ = get_window(window, binsize)
    return fft_(frame(x, binsize, hopsize) * win_, n=binsize, **kwargs)

def istft(X, binsize, hopsize, mode=None, delay=None, **kwargs):

    overlap = float(hopsize) / float(binsize)
    if X.ndim == 2:
        nch, nwin, bins = 1, X.shape
    elif X.ndim == 3:
        nch, nwin, bins = X.shape
    ...
    # This is to account for the delay prior to reconstruction (overlap-add)
    xi = np.concatenate([xi[:,:,delay:], xi[:,:,:delay]], axis=-1) if delay is not None else xi

    # Overlap-Add Reconstruction
    # This is reconstructed with view, should be theoretically faster
    return ola(xi, binsize, hopsize)
